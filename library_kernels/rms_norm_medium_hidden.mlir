// RMSNorm with standard LLM hidden dimension (4096), batch=2048
// Pattern: Llama/GPT hidden dim. One row per block, 128-256 threads.
// tensor<2048x4096xf16> -> tensor<2048x4096xf16>, weights tensor<4096xf16>
// Computation: x * rsqrt(mean(x^2) + eps) * weight
//
// == Optimal Configuration ==
// hidden_dim=4096, batch=2048, f16 input/output, f32 accumulation
// threads_per_row = 256 (one full block handles one row)
// vector_width = 8 (128 bits / 16 bits = 8 f16 elements)
// elements_per_thread = 16 (4096 / 256 = 16, i.e., 2 vector loads)
// rows_per_block = 1 (4096 elems needs all 256 threads)
// workgroup_size = 256
// num_workgroups = 2048 (one per row)
// reduction_strategy: 256 threads -> 8 warps. First reduce within each warp
//   via 5 shuffle rounds (log2(32)=5), then 8 partial sums reduced via
//   shared memory (one write + one read + 3 shuffle rounds for 8 values).
// coalescing: d1 (hidden) is innermost, contiguous. 256 threads * 8 elems
//   * 2 bytes = 4096 bytes per vectorized load. Excellent coalescing.
// register pressure: 16 f16 inputs + 16 f32 intermediates per thread
//   = 32 + 64 = 96 bytes. Comfortable.
// data fits in registers: 16 elements per thread, 2 passes through data
//   (once for x^2 sum, once for normalize). Standard 2-pass RMSNorm.

#pipeline_layout = #hal.pipeline.layout<bindings = [
  #hal.pipeline.binding<storage_buffer>,
  #hal.pipeline.binding<storage_buffer>,
  #hal.pipeline.binding<storage_buffer>
]>
func.func @rms_norm_medium_hidden() {
  %cst = arith.constant 0.000000e+00 : f32
  %cst_eps = arith.constant 9.99999974E-6 : f32
  %cst_n = arith.constant 4.096000e+03 : f32
  %c0 = arith.constant 0 : index
  %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c0) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2048x4096xf16>>
  %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c0) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<4096xf16>>
  %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c0) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2048x4096xf16>>
  %input = iree_tensor_ext.dispatch.tensor.load %0, offsets = [0, 0], sizes = [2048, 4096], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<2048x4096xf16>> -> tensor<2048x4096xf16>
  %weight = iree_tensor_ext.dispatch.tensor.load %1, offsets = [0], sizes = [4096], strides = [1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<4096xf16>> -> tensor<4096xf16>
  %empty_2d = tensor.empty() : tensor<2048x4096xf32>
  %empty_1d = tensor.empty() : tensor<2048xf32>
  %empty_out = tensor.empty() : tensor<2048x4096xf16>

  // Op 1: extf + square
  %squared = linalg.generic {
    indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                     affine_map<(d0, d1) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel"]}
    ins(%input : tensor<2048x4096xf16>) outs(%empty_2d : tensor<2048x4096xf32>) {
  ^bb0(%in: f16, %out: f32):
    %v = arith.extf %in : f16 to f32
    %sq = arith.mulf %v, %v : f32
    linalg.yield %sq : f32
  } -> tensor<2048x4096xf32>

  // Op 2: fill for reduction
  %acc_init = linalg.fill ins(%cst : f32) outs(%empty_1d : tensor<2048xf32>) -> tensor<2048xf32>

  // Op 3: reduce sum of squares along d1
  %sum_sq = linalg.generic {
    indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                     affine_map<(d0, d1) -> (d0)>],
    iterator_types = ["parallel", "reduction"]}
    ins(%squared : tensor<2048x4096xf32>) outs(%acc_init : tensor<2048xf32>) {
  ^bb0(%in: f32, %out: f32):
    %s = arith.addf %in, %out : f32
    linalg.yield %s : f32
  } -> tensor<2048xf32>

  // Op 4: rsqrt(mean + eps)
  %rrms = linalg.generic {
    indexing_maps = [affine_map<(d0) -> (d0)>,
                     affine_map<(d0) -> (d0)>],
    iterator_types = ["parallel"]}
    ins(%sum_sq : tensor<2048xf32>) outs(%empty_1d : tensor<2048xf32>) {
  ^bb0(%in: f32, %out: f32):
    %mean = arith.divf %in, %cst_n : f32
    %shifted = arith.addf %mean, %cst_eps : f32
    %r = math.rsqrt %shifted : f32
    linalg.yield %r : f32
  } -> tensor<2048xf32>

  // Op 5: normalize + weight
  %result = linalg.generic {
    indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>,
                     affine_map<(d0, d1) -> (d0)>,
                     affine_map<(d0, d1) -> (d1)>,
                     affine_map<(d0, d1) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel"]}
    ins(%input, %rrms, %weight : tensor<2048x4096xf16>, tensor<2048xf32>, tensor<4096xf16>) outs(%empty_out : tensor<2048x4096xf16>) {
  ^bb0(%in: f16, %scale: f32, %w: f16, %out: f16):
    %xf = arith.extf %in : f16 to f32
    %wf = arith.extf %w : f16 to f32
    %normed = arith.mulf %xf, %scale : f32
    %weighted = arith.mulf %normed, %wf : f32
    %r = arith.truncf %weighted : f32 to f16
    linalg.yield %r : f16
  } -> tensor<2048x4096xf16>

  iree_tensor_ext.dispatch.tensor.store %result, %2, offsets = [0, 0], sizes = [2048, 4096], strides = [1, 1] : tensor<2048x4096xf16> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<2048x4096xf16>>
  return
}
